...
def business_profile(business_id):
    ...
    return render_template('business/profile.html', business=business, posts=posts, reviews=reviews, user=user, has_reviewed=has_reviewed)

@business.route('/<int:business_id>/posts')
def business_posts(business_id):
    posts = Post.query.filter_by(business_id=business_id).all()
    business = Business.query.get(business_id)
    return render_template('business/post.html', posts=posts, business=business)
